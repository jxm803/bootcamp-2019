...
# Calculate the percent of values in the vector that are missing
mean(is.na(vec))

#### Factors ####

# See lecture notes and DataCamp for guidance and practice - create factors with factor(), which includes an argument for levels =
...
gapminder07$lifeExp_over70 <- NA  # Initialize a variable containing all "NA" values to ensure there will be an NA value if following conditions aren't captured
...
gapminder07$lifeExp_mean <- NA 
gapminder07$lifeExp_mean[gapminder07$lifeExp>67.00742] <- "High"
gapminder07$lifeExp_mean[gapminder07$lifeExp<67.00742] <- "Low"
table(gapminder07$lifeExp_mean)
...
hist(gapminder07$lifeExp,
     main = "Life Expectancy Frequency in 2007",
...
scatter.smooth(gapminder07$gdpPercap, gapminder07$lifeExp)
...
plot(gapminder07$gdpPercap, gapminder07$lifeExp,
     main = "GDP per Capita and Life Expectancy in 2007",
        xlab= "GDP per Capita", 
